...
class GfmfileModel extends AdminModel
{
	...
	public function getItem($pk = null)
	{
		...
		if ($language == 'en' & !empty($eid)) {
			$query = $db->getQuery(true);
			$query->select('a.id AS page_id, a.language, a.jdoc_key, a.path, a.filename, a.display_title')
		...
		if (!empty($eid)) {
			...
			$pattern = '/(.*?\/)\w\w(\/.*)/';
			$test = preg_match($pattern, $item->path, $matches);
			if ($test === 1) {
				$item->path = $matches[1] . $language . $matches[2];
			}
			return $item;
		}
	...
	protected function is_duplicate ($data) {
		...
		if (!empty($id)) {
			...
			$this->setError(Text::_('COM_JDOCMANUAL_GFM_ERROR_DUPLICATE'));
			return true;
		}
		// check also for a valid path and filenemae.
		// ToDo
	...
	protected function commit($data) {
		...
		if (empty(file_put_contents($sitepath, $data['stash']))) {
			// Something went wrong with saving the file.
			Factory::getApplication()->enqueueMessage(Text::_('COM_JDOCMANUAL_GFM_ERROR_GFMSAVE_FAILED'));
			return false;
		}
	}
