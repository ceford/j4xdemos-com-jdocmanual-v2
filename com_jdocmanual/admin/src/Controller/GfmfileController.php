...
class GfmfileController extends FormController
{
    ...
    public function add() {
        $this->app->setUserState('com_jdocmanual.gfmfile.eid', 0);
        $this->app->setUserState('com_jdocmanual.gfmfile.trid', 0);
        parent::add();
    }
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        if ($task == 'pull_request') {
            $validData['pr'] = 1;
        } elseif ($task == 'pull_request_cancel') {
            $validData['pr'] = 0;
        }
        ...
        switch ($task) {
            ...
            case 'commit':
                ...
                $repo_item_path = $data['manual'] . '/' . $data['language'] . '/' . $data['heading'] . '/' . $data['filename'];
                ...
                if (empty(file_put_contents($filepath, $data['markdown_text']))) {
                    $this->setMessage(Text::_('COM_JDOCMANUAL_GFMFILE_GIT_SAVE_FAILED') . ' Path: ' . $filepath);
                } else {
                    $this->setMessage(Text::_('COM_JDOCMANUAL_GFMFILE_GIT_SAVE_SUCCESS'));
                    ...
                    if (!empty ($result)) {
                        $this->app->enqueueMessage(Text::_('COM_JDOCMANUAL_GFMFILE_GIT_ADD_FAILED') . ' Code = ' . $result_code1 . "<br>{$command1}", 'warning');
                    }

                    // Commit the item:
                    $command2 = "cd {$gfmfiles_path}; /usr/bin/git commit -m \"{$data['commit_message']}\" -- {$gfmfiles_path}{$repo_item_path};";
                    $result = exec($command2, $output2, $result_code2);

                    if (empty($result)) {
                        $op = implode('<br>',$output2);
                        $this->app->enqueueMessage(Text::_('COM_JDOCMANUAL_GFMFILE_GIT_COMMIT_FAILED') . " Response:  {$result}<br>Output: {$op}<br>Code: {$result_code2} <br>{$command1}<br>{$command2}", 'warning');
                    } else {
                        ...
                        // For debugging:
                        //$this->setMessage(Text::_('COM_JDOCMANUAL_GFMFILE_GIT_COMMIT_SUCCESS') . " Response:  {$result}<br>Output: {$output2}<br>Code: {$result_code2} <br>{$command1}<br>{$command2}");
                        // For production:
                        $this->setMessage(Text::_('COM_JDOCMANUAL_GFMFILE_GIT_COMMIT_SUCCESS') . " Response:  {$result}");
